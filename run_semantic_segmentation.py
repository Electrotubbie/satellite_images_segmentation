...
# PP
from create_dataset import create_train_val_test_dataset
...
# satellite image project add
@dataclass
class ProjectArguments:
    """
    Arguments for a project on semantic segmentation of satellite images.
    Consist of:
        - parameters for locally dataset load
    """
    
    local_load: Optional[bool] = field( 
        default=True,
        metadata={"help": "Option for load dataset locally."},
    )
    local_dataset_path: Optional[str] = field(
        default='landcover.ai.v1',
        metadata={"help": "Path to dataset for local load."}
    )
    local_dataset_img_path: Optional[str] = field(
        default='images_512',
        metadata={"help": "Path with images in dataset directory."}
    )
    local_dataset_label_path: Optional[str] = field(
        default='labels_512',
        metadata={"help": "Path with labels in dataset directory."}
    )
    local_train_list_file: Optional[str] = field(
        default=None,
        metadata={"help": (
                      'File with names of images and labels for train sample.'
                      'It is better to specify the file name without the extension. For example, for train.txt the name will be "train".'
            )
        }
    )
    local_valid_list_file: Optional[str] = field(
        default=None,
        metadata={"help": (
                      "File with names of images and labels for valid sample."
                      'It is better to specify the file name without the extension. For example, for val.txt the name will be "val".'
            )
        }
    )
    local_test_list_file: Optional[str] = field(
        default=None,
        metadata={"help": (
                      'File with names of images and labels for test sample.'
                      'It is better to specify the file name without the extension. For example, for test.txt the name will be "test".'
            )
        }
    )
...
def main():
    ...
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, TrainingArguments, ProjectArguments)) # satellite image project add
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        # If we pass only one argument to the script and it's the path to a json file,
        # let's parse it to get our arguments.
        model_args, data_args, training_args, project_args = parser.parse_json_file(json_file=os.path.abspath(sys.argv[1]))
    else:
        model_args, data_args, training_args, project_args = parser.parse_args_into_dataclasses()
    ...
    # satellite image project add
    if project_args.local_load:
        samples = dict()
        for file_name in [project_args.local_train_list_file, project_args.local_valid_list_file, project_args.local_test_list_file]:
            if file_name:
                with open(os.path.join(project_args.local_dataset_path, f'{file_name}.txt'), 'r') as f:
                    samples[file_name] = sorted(list(map(lambda x: x.strip(), f.readlines())))
    
        intersection_samples = list(
            (set(samples[project_args.local_train_list_file]) & set(samples[project_args.local_valid_list_file])
          | set(samples[project_args.local_train_list_file]) & set(samples[project_args.local_test_list_file])
          | set(samples[project_args.local_valid_list_file]) & set(samples[project_args.local_test_list_file]))
        )
        if len(intersection_samples) > 0:
            logger.info(f"Train, valid and test samples have intersections between each other")
        dataset = create_train_val_test_dataset(
            img_path=os.path.join(project_args.local_dataset_path, project_args.local_dataset_img_path), 
            label_path=os.path.join(project_args.local_dataset_path, project_args.local_dataset_label_path), 
            train_list=samples[project_args.local_train_list_file],
            val_list=samples[project_args.local_valid_list_file],
            test_list=samples[project_args.local_test_list_file]
        )
        with open(os.path.join(project_args.local_dataset_path, 'id2label.json'), 'r') as f:
